...
public class Console {
		...
		try {
			...
			while (searchBoatData.hasNextLine()) {
				...
				for (int i = 0; i < tempLine1.length(); i++) {
					...
					if (tempLine1.charAt(i) == ',' && commaCounter%3 == 0) {

						int tempMID1 = Integer.valueOf(temp2);
						System.out.println(tempMID1);
						boats.memberID.add(tempMID1);
						temp2 = "";
						commaCounter++;
						i++;
					}

					if (tempLine1.charAt(i) == ',' && commaCounter%3 == 1) {
					...
					if (tempLine1.charAt(i) == ',' && commaCounter%3 == 2) {
					...
					if (tempLine1.charAt(i) == ';'&& commaCounter%3 == 0) {
						...
						String tempLength = temp2;
						boats.boatLength.add(Integer.parseInt(tempLength));
						temp2 = "";
	...
	public void registerBoat(Model.Member member, Model.Boat boats, Scanner userInput){
		...
		boats.boatID.add(boats.UniqueBoatID());
		System.out.println(
	...
	public void deleteBoat(Model.Member member, Model.Boat boats, Scanner userInput){
		...
		for (int i = 0; i < boats.typeOfBoats.size(); i++) {
			if (boatIDForDelete == boats.boatID.get(i)) {
				...
				member.numOfBoats.set(x, member.numOfBoats.get(i)-1);
			}
		}
		/*for (int i = 0; i < member.memberInfo3.size(); i++) {
			...
			if (tempNumberOfBoats >= 1) {
				for (int x = 0; x < boats.memberID.size(); x++) {
					...
					System.out.println("Boat" + BoatCounter + ": " + "BoatID: " + boats.boatID.get(x) + "     "
							+ "Type: " + boats.typeOfBoats.get(x) + "     " + "Length: "+boats.boatLength.get(x)+"\n");
					}
				}
			}
		}
